...
class TestFiles(TestCase):
    ...
    def test_file_count(self):
        file_count = len(fnmatch.filter(os.listdir(TestFiles.GRCH37_PATH), '*.fa'))
...
class TestInvalidQueries(TestCase):
    ...
    def test_invalid_start_position(self):
        with self.assertRaises(ValueError):
            Chromosome(21).sequence(-1, 10)

    def test_invalid_end_position(self):
        with self.assertRaises(ValueError):
            Chromosome(21).sequence(123457, 123456)

    def test_out_of_range_start_position(self):
        with self.assertRaises(ValueError):
            Chromosome(15).sequence(102531393, 102531394)

    def test_out_of_range_end_position(self):
        with self.assertRaises(ValueError):
            Chromosome(15).sequence(102531380, 102531394)
